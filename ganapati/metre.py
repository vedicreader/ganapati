...
from .text import verse_spans, line_times
...
def mora_weights(text:str) -> L:
    'The syllables as `(syllable, morae)` pairs — 2 for a guru, 1 for a laghu. What duration work needs.'
    return L((s, 2 if h else 1) for s, h in syllables(text))
...
# ardhasamavṛtta: the odd pādas take one pattern, the even another, so the halves differ in length
ARDHASAMA = {
    'viyoginī':   ('sa sa ja ga', 'sa bha ra la ga'),   # 10 + 11
    'puṣpitāgrā': ('na na ra ya', 'na ja ja ra ga'),    # 12 + 13
}
_ARDHA = {k: (gana_pattern(a), gana_pattern(b)) for k, (a, b) in ARDHASAMA.items()}

_METER_NAMES = [*_PAT, *_MIXED, 'anuṣṭubh', *ARDHASAMA, *MATRA_METERS]
...
# %% ../nbs/01_metre.ipynb #c5b93c98
def detect_ardhasama(text:str, weights:list=None):
    'The ardhasamavṛtta of a verse — `AttrDict(name, padas, ganas)` — or None.'
    w = weights if weights is not None else [h for _, h in syllables(metrical_text(text))]
    for nm, (a, b) in _ARDHA.items():
        la, lb = len(a), len(b)
        if len(w) != 2 * (la + lb): continue
        qs = (w[:la], w[la:la + lb], w[la + lb:2 * la + lb], w[2 * la + lb:])
        if all(_pada_ok(q, p) for q, p in zip(qs, (a, b, a, b))):
            return AttrDict(name=Meter(nm), padas=(la, lb), ganas=' | '.join(ARDHASAMA[nm]))
    return None

def detect_meter(text:str):
    'One verse as `AttrDict(name, variant, syllables, per_pada, ganas, scan, matras, halves)`.'
    w = [h for _, h in syllables(metrical_text(text))]
    n = len(w)
    if not n: return None                           # the only None: nothing scanned at all
    ...
    if (a := detect_ardhasama(text, w)):
        return AttrDict(name=a.name, variant=None, syllables=n, per_pada=None,
                        ganas=a.ganas, scan=sc, matras=mt, halves=None)
    ...
    q = n // 4 if n >= 8 and not n % 4 else None
...
# %% ../nbs/01_metre.ipynb #557b7af5f428
_GL = frozenset('gl')

def _weights(pada:str) -> list:
    'Weights for a pāda given either as a `g`/`l` pattern or as Sanskrit text.'
    s = (pada or '').strip()
    if s and set(s) <= _GL: return [c == 'g' for c in s]
    return [h for _, h in syllables(metrical_text(s))]

def _anustubh_pada(w) -> bool:
    'Whether eight weights could be a śloka pāda, in the odd position or the even.'
    k = ''.join('g' if x else 'l' for x in w[4:7])
    return k == 'lgl' or k == _PATHYA or k in _VIPULA

def match_pada(pada:str) -> L:
    'The metres one pāda could belong to — what `detect_meter` cannot say from a partial verse.'
    w = _weights(pada)
    if not w: return L()
    out = L(Meter(nm) for nm, pat in _PAT.items() if _pada_ok(w, pat))
    if len(w) == 8 and _anustubh_pada(w): out.append(Meter('anuṣṭubh'))
    return out

def group_verses(lines,             # the lines of a source, or one string of them
                 sizes=(1, 2, 4)    # lines per verse to try, in order
                ):
    'The line grouping that names the most metres, as `AttrDict(size, coverage, verses, meters)`.'
    ls = L(lines.splitlines() if isinstance(lines, str) else lines).map(str.strip).filter()
    best = AttrDict(size=0, coverage=0.0, verses=L(), meters=L())
    for k in sizes:
        if k > len(ls): continue
        vs = L(' '.join(ls[i:i+k]) for i in range(0, len(ls), k))
        ms = vs.map(detect_meter)
        cov = sum(bool(m and m.name) for m in ms) / len(vs)
        if cov > best.coverage: best = AttrDict(size=k, coverage=cov, verses=vs, meters=ms)
    return best


# %% ../nbs/01_metre.ipynb #e55b668be445
TIMING_TOL = 0.35

def _rates(ts) -> list:
    'The `(line, ms per mora)` of every timed line that scans.'
    return [(t, t.ms / m) for t in ts if (m := matras(metrical_text(t.text)))]

def _median(xs):
    s = sorted(xs)
    return s[len(s)//2] if s else None

def mora_rate(text:str) -> float:
    'Milliseconds per mora over a chunk\'s timed lines, or None when the chunk carries no timings.'
    return _median([r for _, r in _rates(line_times(text))])

def timing_check(text:str, tol:float=TIMING_TOL) -> L:
    'Timed lines more than `tol` off the chunk median in ms per mora — audio and scansion disagree.'
    rs = _rates(line_times(text))
    med = _median([r for _, r in rs])
    if not med or len(rs) < 3: return L()
    return L(t for t, r in rs if abs(r - med) / med > tol)

def guru_laghu_ratio(texts) -> float:
    'Guru:laghu duration over many timed lines, by least squares; None when the lines cannot fix it.'
    ts = L([texts] if isinstance(texts, str) else texts).map(line_times).concat()
    gg = gl = ll = gy = ly = 0.0
    for t in ts:
        w = [h for _, h in syllables(metrical_text(t.text))]
        if not w: continue
        ng, nl, y = float(sum(w)), float(len(w) - sum(w)), float(t.ms)
        gg += ng*ng; gl += ng*nl; ll += nl*nl; gy += ng*y; ly += nl*y
    if not (det := gg*ll - gl*gl): return None
    dg, dl = (gy*ll - gl*ly) / det, (gg*ly - gl*gy) / det
    return dg / dl if dl > 0 else None
...
def verse_meta(text:str) -> dict:
    ...
    rs = _rates(line_times(text))
    if (med := _median([r for _, r in rs])):
