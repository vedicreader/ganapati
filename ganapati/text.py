...
# %% auto #0
__all__ = ['PUNCT', 'VERSE_TARGET', 'VERSE_MAX', 'TIME_RE', 'ETYM_RE', 'verse_spans', 'is_word', 'deva_num', 'split_mantras',
           'padas', 'chunk_verses', 'VerseChunker', 'ProseChunker', 'gretil_parse', 'tei_parse', 'time_marker',
           'line_times', 'line_etyms', 'vr_xml_parse', 'vr_json_parse', 'dcs_parse', 'is_sanskrit', 'sanskrit_parse']
...
def is_word(t:str) -> bool:
    'A token with a letter in it: a daṇḍa, a verse number or a lone punctuation mark is not a word.'
    return any(c.isalpha() for c in t or '')

def deva_num(n) -> str:
    'A verse number in Devanagari digits: `18` -> `१८`, `1.1` -> `१.१`.'
    return str(n).translate(_NUM2DEVA)

def split_mantras(text:str) -> L:
    'A text blob as `(number, body)` pairs, cut at its `॥ n ॥` end-markers, the number as written.'
    out, last = L(), 0
...
def padas(verse:str,    # one verse, on one line or several, with or without its end-marker
    ...
    ps = [p.strip() for p in re.split(r'[।|]', t) if p.strip()]
    end = f' {DDANDA} {deva_num(num)} {DDANDA}' if num is not None else f' {DDANDA}'
    return out + L(f'{p} {DANDA}' if i < len(ps) - 1 else f'{p}{end}' for i, p in enumerate(ps))
